...
|---|---|---|---|---|---|---|---|
| T001 | /stats 聚合契约：树+维度+甘特+偏差+关键路径+源链接+状态漂移 | Must Deliver | — | 否 | 已完成 | 35k–60k | `cd telemetry && bun test`；`bash project-template/bin/fpg-check.sh plan-lint docs/iteration/epics/E002-metrics-refinement/sprints/S002-dashboard-drilldown/plan.md` |
| T002 | 看板单页（原生 JS）：总览/下钻/开发者/视角 + 漂移徽标 | Must Deliver | T001 | 是(与 T003) | 已完成 | 30k–60k | `cd telemetry && bun test`；`bash project-template/bin/fpg-check.sh plan-lint docs/iteration/epics/E002-metrics-refinement/sprints/S002-dashboard-drilldown/plan.md` |
| T003 | 关注/视角 prefs：/api/prefs 路由 + 看板控件接线 | Must Verify | T001 | 是(与 T002) | 已完成 | 15k–35k | `cd telemetry && bun test`；`bash project-template/bin/fpg-check.sh plan-lint docs/iteration/epics/E002-metrics-refinement/sprints/S002-dashboard-drilldown/plan.md` |
| T004 | 状态一致性闸门：fpg-check 父子状态矛盾检测（纯 plan、机械） | Must Verify | — | 是(与 T001/T002/T003) | 已完成 | 10k–20k | `cd telemetry && bun test`；`bash project-template/bin/fpg-check.sh plan-lint docs/iteration/epics/E002-metrics-refinement/sprints/S002-dashboard-drilldown/plan.md` |
