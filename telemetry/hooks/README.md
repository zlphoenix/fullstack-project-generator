...
2. **E/S/T 归因**：读 `<cwd>/.fpg/current-task`（k=v 每行，由 Skill 在切片开始写、结束删）：
   `project_root` 取 hook payload 的 `cwd`，`epic/sprint/task/story/platform` 与可选的 `epic_name/sprint_name/task_name`、`epic_path/sprint_path/task_path` 并入 attrs；`skill/phase/milestone` 覆盖事件同名字段。没有标记文件时不再把 `codex/claude` 写成 Skill，避免 Agent 与 Skill 维度混淆。
